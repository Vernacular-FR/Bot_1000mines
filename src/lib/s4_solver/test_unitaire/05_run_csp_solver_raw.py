...
from src.lib.s4_solver.s41_propagator_solver.s411_frontiere_reducer import PropagationResult  # noqa: E402
...
def process_screenshot(screenshot: Path) -> None:
    ...
    # Étape CSP PURE : pas de reducer préalable, juste la frontière issue du classifier
    def _blank(reason: str) -> PropagationResult:
        return PropagationResult(set(), set(), set(), 0, reason)

    pipeline_stub = PropagatorPipelineResult(
        safe_cells=set(),
        flag_cells=set(),
        iterative=_blank("iter-skip"),
        subset=_blank("subset-skip"),
        advanced=_blank("adv-skip"),
        iterative_refresh=_blank("iter-refresh-skip"),
    )

    csp_manager = CspManager(view, cells)
    csp_manager.run_pure(pipeline_stub)
    ...
    # Créer les actions (CSP uniquement) pour l'overlay
    ...
    print(
        f"[CSP] {screenshot.name}: csp_safe={len(csp_safe)} csp_flags={len(csp_flags)} "
        f"zones={len(csp_manager.segmentation.zones) if csp_manager.segmentation else 0} "
