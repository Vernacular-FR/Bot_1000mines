...
- Gérer les statuts solver (`JUST_REVEALED`, `ACTIVE`, `FRONTIER`, `SOLVED`, `NONE`, `OUT_OF_SCOPE`) et `action_status` (SAFE / FLAG / LOOKUP) tout en restant passif.
...
- **Stockage passif** : Vision pousse revealed+`JUST_REVEALED`, Solver calcule frontier_add/remove et reclasse ACTIVE/FRONTIER/SOLVED, storage ne fait que stocker.
...
- **GridCell**
  ...
  - `solver_status` (`JUST_REVEALED`, `ACTIVE`, `FRONTIER`, `SOLVED`, `NONE`, `OUT_OF_SCOPE`)
  - `action_status` (`SAFE`, `FLAG`, `LOOKUP`, `NONE`), `source` (vision / solver), `updated_at`.
