...
class Segmentation:
    ...
    def __init__(self, analyzer: GridAnalyzer, tensor_context: Optional[SolverContext] = None):
        ...
        if tensor_context:
            self._build_from_tensor_context(tensor_context)
        else:
            self._run()
    ...
    def _build_from_tensor_context(self, context: SolverContext) -> None:
        zone_id = 0
        for fzone in context.frontier_zones:
            zone = Zone(
                id=zone_id,
                cells=list(fzone.unknown_cells),
                constraints=list(fzone.number_cells.keys()),
            )
