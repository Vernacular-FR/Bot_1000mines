...
class TensorGrid:
    ...
    def _current_bounds(self) -> GridBounds:
        x_min = -self.offset_x
        y_min = -self.offset_y
        x_max = x_min + self.shape[1] - 1
        y_max = y_min + self.shape[0] - 1
    ...
    def get_solver_view(self, bounds: Optional[GridBounds] = None) -> Dict[str, np.ndarray]:
        ...
        if bounds is None:
            ...
            view_bounds = self._current_bounds()
        else:
            region_slice = self._ensure_bounds(bounds)
            view = {
        ...
        view["origin"] = (view_bounds[0], view_bounds[1])
