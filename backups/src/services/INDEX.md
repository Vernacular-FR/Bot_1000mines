...
- **dependencies**: lib.s1_interaction.game_controller, lib.s1_interaction.coordinate_system
...
- **dependencies**: lib.s1_interaction.browser_manager, lib.s1_interaction.game_controller
...
- **dependencies**: lib.s1_interaction.screenshot_manager, lib.s1_interaction.combined_overlay, lib.s1_interaction.interface_detector
...
- **dependencies**: lib.s1_interaction.screenshot_manager, lib.s2_analysis.grid_state (GamePersistence + GridDB)
- **features**: Reconnaissance IA, remplissage du `grid_db.json` via `GamePersistence`, génération d'overlays conditionnelle
...
- **dependencies**: lib.s2_analysis.grid_state (GridDB), lib.s3_solver.csp_engine
- **features**: Backtracking, segmentation en zones, overlays solveur
...
- **dependencies**: lib.s1_interaction.game_controller (API `execute_game_action`), lib.s1_interaction.coordinate_system
...
- **dependencies**: ZoneCaptureService, OptimizedAnalysisService, GameSolverService, ActionExecutorService, lib.s2_analysis.grid_state.GamePersistence
...
│   │   └── ScreenshotManager + CombinedOverlay (lib.s1_interaction)
│   ├── OptimizedAnalysisService (analyse IA + GridDB)
│   │   └── GamePersistence / GridDB (lib.s2_analysis)
│   ├── GameSolverService (résolution CSP)
│   │   ├── GridDB (lib.s2_analysis)
│   │   └── CSPEngine (lib.s3_solver.csp_engine)
│   ├── ActionExecutorService (exécution des actions)
│   │   └── MineSweeperBot.execute_game_action (lib.s1_interaction)
│   ├── GameLoopService (orchestration complète)
│   │   ├── ZoneCaptureService
│   │   ├── OptimizedAnalysisService
│   │   ├── GameSolverService
│   │   ├── ActionExecutorService
│   │   └── GamePersistence (lib.s2_analysis)
...
│   ├── game_controller.py / coordinate_system.py
│   ├── screenshot_manager.py + combined_overlay.py
│   └── interface_detector.py
├── s2_analysis/
│   ├── grid_state.py (GridState + GamePersistence + GridDB)
│   └── template_matching_fixed.py / mapper.py
└── s3_solver/
    ├── core/ (GridAnalyzer, Segmentation...)
    ├── csp/
    └── visualization/
```
