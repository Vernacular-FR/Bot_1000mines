...
class Frontier:
    ...
    def __init__(self, grid_state: GridState):
        ...
        frontier_hint = grid_state.get_frontier_hint()
        if frontier_hint:
            self._build_from_hint(frontier_hint)
        else:
            self._build()
    ...
    def _build_from_hint(self, frontier_cells: Set[Tuple[int, int]]):
        for ux, uy in frontier_cells:
            constraints = self._get_number_neighbors(ux, uy)
            if not constraints:
                continue
            self.cells.add((ux, uy))
            self.constraints[(ux, uy)] = constraints

    def _get_number_neighbors(self, x: int, y: int) -> List[Tuple[int, int]]:
        neighbors = []
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx == 0 and dy == 0:
                    continue
                nx, ny = x + dx, y + dy
                val = self.grid_state.get_cell(nx, ny)
                if val is not None and 0 <= val <= 8:
                    neighbors.append((nx, ny))
        return neighbors
