...
class Segmentation:
    ...
    def __init__(self, analyzer: GridAnalyzer, tensor_context: Optional[SolverContext] = None):
        ...
        self._allowed_frontier: Optional[Set[Tuple[int, int]]] = None

        if tensor_context:
            self._allowed_frontier = set()
            for fzone in tensor_context.frontier_zones:
                self._allowed_frontier.update(fzone.unknown_cells)
    ...
    def _create_zones(self):
        ...
        for cell in frontier_cells:
            if self._allowed_frontier is not None and cell not in self._allowed_frontier:
                continue
