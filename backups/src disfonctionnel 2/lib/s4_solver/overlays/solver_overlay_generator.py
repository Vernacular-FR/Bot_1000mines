...
class SolverOverlayGenerator:
    ...
    def __init__(self, cell_size: int = CELL_SIZE, output_dir: str | None = None):
    ...
    def generate_overlay_from_db(self, screenshot_path: str, grid_db: GridDB, 
        ...
        for cell in cells:
            ...
            # Texte centré pour les nombres
            if isinstance(cell_type, str) and cell_type.startswith("number_"):
                number = cell_type.split("_")[1]
                text_color = (255, 255, 255) if cell_type in ["number_7", "number_8", "mine"] else (0, 0, 0)
                font = self.fonts.get("medium", ImageFont.load_default())

                # Position approximativement centrée
                text_pos = (pixel_x + 8, pixel_y + 6)
                draw.text(text_pos, number, fill=text_color + (255,), font=font)

            # Afficher les probabilités (pour unknown et unrevealed)
            if cell_type in ["unknown", "unrevealed"]:
                metadata = cell.get("metadata", {})
                if "mine_probability" in metadata:
                    prob = metadata["mine_probability"]
                    prob_txt = f"{int(prob*100)}"
                    font = self.fonts.get("small", ImageFont.load_default())
                    # Noir pour lisibilité
                    draw.text((pixel_x + 4, pixel_y + 6), prob_txt, fill=(0, 0, 0, 255), font=font)
