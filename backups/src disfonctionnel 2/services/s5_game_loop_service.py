...
from .s2_smart_scan_service import SmartScanService, CapturePatch
...
class GameLoopService:
    def __init__(self, session_service: SessionSetupService, 
        ...
        self.smart_scan_service = SmartScanService(paths=self.game_paths, enable_grid_db=False)
        self.solver_service = GameSolverService(paths=self.game_paths, generate_overlays=False)
    ...
    def execute_single_pass(self, iteration_num: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, "coordinate_converter") and self.coordinate_converter:
                self.coordinate_converter.set_grid_origin(zone_bounds[0], zone_bounds[1])
            
            patch = CapturePatch(
                image_path=capture_result['zone_path'],
                bounds=zone_bounds,
                usable_mask=capture_result.get('usable_mask'),
            )
            scan_result = self.smart_scan_service.process_patches([patch])

            if scan_result.get('updates', 0) <= 0:
                pass_result['message'] = "SmartScan n'a produit aucune mise à jour"
                return pass_result

            analysis_result = {
                'success': True,
                'grid_bounds': zone_bounds,
                'cell_count': scan_result.get('summary', {}).get('total_cells', 0),
                'overlay_path': None,
                'game_status': {'symbol_distribution': scan_result.get('symbol_distribution', {})},
                'summary': scan_result.get('summary', {}),
                'message': 'SmartScan OK',
            }
            ...
            # Ajouter le fichier d'analyse créé (plus de grid_db lorsque Tensor-only)
            ...
            solve_result = self.solver_service.solve_from_tensor(
                scan_result,
    ...
    def _update_dirty_bounds(self, dirty_sets: Optional[List[Any]]) -> None:
        merged = self._merge_dirty_sets(dirty_sets)
        self.last_dirty_bounds = merged

    def _merge_dirty_sets(self, dirty_sets: Optional[List[Any]]) -> Optional[Tuple[int, int, int, int]]:
        if not dirty_sets:
            return None
        ...
        for dirty in dirty_sets:
            bounds = getattr(dirty, "bounds", None)
            if bounds is None and isinstance(dirty, dict):
                bounds = dirty.get("bounds")
            if not bounds:
                continue
    ...
    def _next_capture_bounds(self) -> Optional[Tuple[int, int, int, int]]:
        if not self.last_dirty_bounds:
            return None
        x_min, y_min, x_max, y_max = self.last_dirty_bounds
        padding = 1
        return (
            x_min - padding,
            y_min - padding,
