#!/usr/bin/env python3
"""
Service SmartScan : consomme CapturePatch, écrit dans TensorGrid (option GridDB).
Dirty sets non exploités pour l'instant (hint cache non utilisé).
"""
...
class SmartScanService:
    def __init__(
        ...
        *,
        ...
        if self.enable_grid_db:
            if not self.grid_db_path:
                raise ValueError("grid_db est requis lorsque enable_grid_db=True")
    ...
    def process_patches(self, patches: List[CapturePatch]) -> Dict[str, object]:
        ...
        response = {
            "tick_id": tick_id,
            "updates": updates,
            "dirty_sets": [],  # Dirty sets désactivés pour le moment
            "summary": summary,
            "symbol_distribution": symbol_distribution,
            "metrics": metrics,
        }
        response["db_path"] = self.grid_db_path if self.enable_grid_db else None
        return response
