...
class GameSolverService:
    ...
    def __init__(self, generate_overlays: bool = True, paths: Dict[str, str] = None):
        ...
        if generate_overlays and not self.solver_overlays_path:
            raise ValueError("solver path requis pour générer des overlays")
    ...
    def solve_from_db_path(self, db_path: Optional[str] = None,
        ...
        try:
            ...
            self._log_tensor_view_summary(tensor_view, consumed_events, source="GridDB")
    ...
    def solve_from_tensor(
        ...
        *,
        ...
        """
        Résoudre directement à partir du TensorGrid (pipeline Tensor-only).
        """
        start_time = time.time()
        try:
            tensor_view, consumed_events = self._prepare_solver_view(full_view=True)
            tensor_context = self._extract_tensor_context(tensor_view, consumed_events)
            self._log_tensor_view_summary(tensor_view, consumed_events, source="TensorGrid")
            analyzer = GridAnalyzer(None, tensor_view=tensor_view)
            solver = HybridSolver(analyzer, tensor_context=tensor_context)

            print("[SOLVE] Résolution TensorGrid (aucune GridDB)")
            solve_start = time.time()
            solver.solve()
            solve_duration = time.time() - solve_start
            metrics = self._compute_solver_metrics(
            ...
            return {
                'success': True,
                'message': 'Résolution TensorGrid terminée',
                'statistics': {
                    'safe_cells': len(safe_cells),
                    'flag_cells': len(flag_cells),
                    'total_actions': total_cells,
            ...
            import traceback
            error_msg = f"Erreur lors de la résolution Tensor: {str(e)}"
            print(f"[ERREUR] {error_msg}")
            traceback.print_exc()
            return {
                'success': False,
    ...
    def _prepare_solver_view(
        self, *, full_view: bool = False
    ) -> Tuple[Dict[str, Any], List[HintEvent]]:
        events = self.tensor_runtime.hint_cache.fetch()
        bounds = None if full_view else self._merge_bounds_from_events(events)
        if bounds:
    ...
    def _log_tensor_view_summary(
        ...
        *,
        ...
        values = tensor_view["values"]
        frontier_mask = tensor_view.get("frontier_mask")
        bounds = tensor_view.get("bounds")
        origin = tensor_view.get("origin", (0, 0))
        ...
        unknown_count = int(np.count_nonzero(values == unknown_code))
        flag_count = int(np.count_nonzero(values == flag_code))
        number_codes = [TensorGrid.encode_cell_type(getattr(CellType, f"NUMBER_{i}")) for i in range(1, 9)]
        number_count = int(np.count_nonzero(np.isin(values, number_codes)))
        frontier_count = int(np.count_nonzero(frontier_mask)) if frontier_mask is not None else 0

        print(
            "[S4][TensorView] "
            f"source={source} bounds={bounds or 'full'} origin={origin} shape={values.shape} "
            f"total={total_cells} unknown={unknown_count} numbers={number_count} flags={flag_count} "
            f"frontier={frontier_count} events={len(events)}"
        )
