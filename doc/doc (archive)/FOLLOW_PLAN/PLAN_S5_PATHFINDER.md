...
- Exploiter la liste des cellules **à re-visualiser** (`TO_VISUALIZE`, écrit par le solver) pour cadrer la re-capture.
...
1) **Solver (s4)** → s5 : liste d’actions (CLICK/FLAG/GUESS). La décision de bypass CSP (réduction suffisante vs CSP) appartient à s4.
2) **s5 ActionPlanner** :
   - ordonne
   - transforme les safe-click en double-clic DOM (deux actions successives)
   - peut ajouter des clics opportunistes de “ménage” local (heuristique), sans dépendre du détail des focus levels
   - produit un `PathfinderPlan`
   - consomme `TO_VISUALIZE` (écrit par s4) pour cadrer la re-capture
   - consomme deux listes si fournies : `actions` (solver) + `cleanup_actions` (bonus quand `enable_cleanup` est ON côté solver)
