...
- Lire le snapshot storage (cells + sets) et produire des actions :
  - `CLICK` (safe)
  - `FLAG` (mine certaine)
  - `GUESS` (si nécessaire, contrôlé par `allow_guess`, défaut OFF (!!!))
...
- Entrées typiques :
  - `active_set` (cells révélées exploitables localement)
...
Responsabilité : exécuter une réduction de frontière déterministe (toujours), puis résoudre les zones frontier “dures” via CSP si nécessaire.
...
- Sortie :
  ...
  - `StorageUpsert` (au minimum `frontier_add/remove`, `active_remove`, métadonnées)
- Options runtime pilotées par l’orchestrateur : `allow_guess` (émission de GUESS si aucune action sûre), `enable_cleanup` (génération de la liste bonus `cleanup_actions` après actions solver).
...
- Entrée : snapshot venant de s3 (`get_cells(bounds)` + `get_frontier()` + `get_active()`)
- Sortie :
  - `actions: list[SolverAction]`
  - `reducer_actions: list[SolverAction]` (si présent)
  - `storage_upsert: StorageUpsert` (active/frontier/metadatas)
...
Schéma de flux (vue logique) :
 ...
 s41_propagator_solver (futur)   s42_csp_solver (CSP+reducer)
     ...
     Sortie s4 (facade.py) : actions + storage_upsert
        ...
        - StorageUpsert: active_remove + frontier_add/remove + metadatas
 ```

1) **Vision → Storage** : `StorageUpsert` avec `cells` + `revealed_add`
2) **SolverController** :
   - lit `active` + `frontier` + cells
   - s40 : calcule les vues (ACTIVE/FRONTIER/SOLVED)
   - s42 : réduction de frontière (toujours) + CSP (optionnel, sur zones `TO_PROCESS`)
3) **Solver → Storage** : publie `StorageUpsert` (active_remove, frontier_add/remove, métadonnées, to_visualize)
4) **Solver → s5_actionplanner** : publie `SolverAction` (CLICK/FLAG/GUESS)
   - Actions principales + liste bonus `cleanup_actions` (si `enable_cleanup` activé)
...
3) **Phase 3 – CSP en bloc**
   ...
   - N’exécuter le CSP que quand la réduction de frontière ne produit pas assez d’actions et que le volume `TO_PROCESS` le justifie (seuils définis plus tard)
4) **Phase 4 – Overlays**
