...
- **Stockage passif** : 
  1. Vision écrit `raw/logical` + `JUST_VISUALIZED` sur les nouvelles obs (FLAGS compris), alimente `revealed_set` uniquement.
  2. State analyzer reclasse `JUST_VISUALIZED → ACTIVE/FRONTIER/SOLVED` et promeut les voisins (ACTIVE REDUCED→TO_REDUCE, FRONTIER zone PROCESSED→TO_PROCESS).
  3. Solver marque SAFE → `TO_VISUALIZE` (logical/réel restent UNREVEALED) et FLAG → SOLVED ; publie `to_visualize` dans l’upsert.
  4. Options solver (pilotées en s4) : `allow_guess` (autorise GUESS) et `enable_cleanup` (génère la liste bonus cleanup) – storage reste passif.
- **Set revealed** / **known_set** : coordonnées des cellules déjà vues (optimisation Vision, évite re-scan).
- **Set active** : cellules révélées numérotées exploitables localement (OPEN_NUMBER avec voisins UNREVEALED) **∪ `to_visualize`** (SAFE cliquées à re-capturer).
- **Set frontier** : cellules UNREVEALED adjacentes à des ACTIVE (frontière de contraintes).
- **to_visualize** : coordonnées SAFE marquées par le solver pour re-capture.
...
- **GridCell**
  ...
  - `focus_level` :
    - ACTIVE : `ActiveRelevance = TO_REDUCE / REDUCED`
    - FRONTIER : `FrontierRelevance = TO_PROCESS / PROCESSED` (homogène par zone)
