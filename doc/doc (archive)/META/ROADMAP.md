...
- Nomenclature FocusLevel : `TO_REDUCE/REDUCED` et `TO_PROCESS/PROCESSED`.
- `TO_VISUALIZE` : écrit par le solver lors des décisions `SAFE`, consommé ensuite pour cadrer la re-capture.
- ZoneDB formalisée comme index dérivé (pilotage CSP via zones `TO_PROCESS`).
- Référence dumb solver loop consolidée dans `doc/FOLLOW_PLAN/s44_dumb_solver_loop.md`.
